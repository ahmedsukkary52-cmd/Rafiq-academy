...
**Release Readiness target state:** versioned per-collection role/ownership rules; Storage path rules; deploy wiring for rules/indexes; reconciled composite-index manifest; CI (`flutter analyze` + `flutter test`); documented release command and environment ownership.
