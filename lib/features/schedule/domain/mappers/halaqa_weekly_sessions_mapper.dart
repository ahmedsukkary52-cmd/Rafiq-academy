...
///
/// Domain-owned (H2 / A-H13) — use cases must not import schedule data mappers.
class HalaqaWeeklySessionsMapper {
  const HalaqaWeeklySessionsMapper();

  List<ClassSessionEntity> map(
    HalaqaScheduleSource source, {
    DateTime? now,
  }) {
  ...
  List<TodayOperationalDay> mapTodayOperationalDays(
    Iterable<HalaqaScheduleSource> sources, {
    DateTime? now,
  }) {
    final clock = now ?? DateTime.now();
    ...
    for (final source in sources) {
      ...
      for (final session in map(source, now: clock)) {
        ...
        final endAt = session.endAt.isAfter(existing.endAt)
            ? session.endAt
            : existing.endAt;
        byHalaqa[halaqaId] = ClassSessionEntity(
          id: existing.id,
