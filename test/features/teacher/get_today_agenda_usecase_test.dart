...
import 'package:rafiq_academy/features/teacher/domain/entities/teacher_day_agenda.dart';
...
void main() {
  group('GetTodayAgendaUseCase', () {
    ...
    test(
      'both signals appear in stable order (attendance then reviews)',
      () async {
        final repo = _FakeTeacherRepository(
          recitations: {
            'h1': [_recitation('h1', pending: true)],
    ...
    test('completed work is removed but still counted as a session', () async {
      final repo = _FakeTeacherRepository(
        ...
        recitations: {
          'h1': [_recitation('h1', pending: false)],
        },
      );
      final useCase = GetTodayAgendaUseCase(repo);
    ...
    test('multiple halaqat ordered by start time then id (D7)', () async {
      final useCase = GetTodayAgendaUseCase(_FakeTeacherRepository());
